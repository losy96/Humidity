...
public class ForgetPassword extends AppCompatActivity {
    ...
    private HttpGetHumidity forgetPassword = new HttpGetHumidity();
    @Override
...
//                    Toast.makeText(getApplicationContext(),"邮件格式错误",Toast.LENGTH_SHORT).show();
        ...
        changePassword.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (isEmail(userIdText.getText().toString())&&isEnoughLen(newPasswordText.getText().toString())){
                    new Thread(networkTask).start();
                    Toast.makeText(getApplicationContext(),"更改密码",Toast.LENGTH_SHORT).show();
                }
                else {
//                    Toast.makeText(getApplicationContext(),"邮件格式错误",Toast.LENGTH_SHORT).show();
                }
    ...
    public String getRequestURL(int requestType){
        String tempURL = null;
        if (requestType == 0){
            tempURL = "http://120.79.52.102/forget_password_code/?uid="+userIdText.getText().toString();
        }
        else if (requestType == 1){
            String npwd = MD5.getMD5(newPasswordText.getText().toString());
            String cd = codeText.getText().toString();
            tempURL = "http://120.79.52.102/forget_password_change/?uid="+userIdText.getText().toString()+"&cd="+cd+"&npwd="+npwd;
            Log.i("URL",tempURL);
        }
        return tempURL;
    }
    ...
    Runnable networkTask = new Runnable() {
        ...
        public void run() {
            ...
            //TODO
            try {
                String userString = forgetPassword.main(getRequestURL(requestType));
